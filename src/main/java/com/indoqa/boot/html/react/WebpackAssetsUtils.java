...
final class WebpackAssetsUtils {
    ...
    private static volatile ClassPathScanner classpathScanner;

    private WebpackAssetsUtils() {
        // hide utility class constructor
    }

    static void findWebpackAssetsInClasspath(String mountPath, String folder, Consumer<String> setMainCss,
        Consumer<String> setMainJavascript) {
        try {
            setupScanner();
            Set<URL> resources = classpathScanner.findFiles(createClasspathAssetsPattern(folder, mountPath));
            findFirstResource(resources, folder, FILE_SUFFIX_CSS, setMainCss);
            findFirstResource(resources, folder, FILE_SUFFIX_JS, setMainJavascript);
        } catch (IOException e) {
            throw new ApplicationInitializationException("Error while searching for webpack assets.", e);
        }
    }

    static void findWebpackAssetsInFilesystem(String assetsFolder, String folder, Consumer<String> setMainCss,
        Consumer<String> setMainJavascript) {
        Set<URL> resources = findLocalFiles(assetsFolder);
        findFirstResource(resources, folder, FILE_SUFFIX_CSS, setMainCss);
        findFirstResource(resources, folder, FILE_SUFFIX_JS, setMainJavascript);
    }
    ...
    private static void findFirstResource(Set<URL> resources, String basePath, String suffix, Consumer<String> consumer) {
        resources
            .stream()
            .map(URL::getPath)
            .filter(path -> path.endsWith(suffix))
            .map(path -> substringAfterLast(path, basePath))
            .filter(path -> countMatches(path, '.') == 1)
            .findFirst()
            .ifPresent(consumer);
    }
